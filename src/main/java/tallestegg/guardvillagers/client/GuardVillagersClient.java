...
public class GuardVillagersClient implements ClientModInitializer {
    ...
    public void onInitializeClient() {
        // Register model layers (Fabric 26.1.2: EntityModelLayerRegistry → ModelLayerRegistry)
        ...
        EntityRendererRegistry.register(GuardEntityType.GUARD, GuardRenderer::new);

        // Register client networking receivers
        // Fabric 26.1.2: CustomPacketPayload handler API changed
        ClientPlayNetworking.registerGlobalReceiver(GuardOpenInventoryPacket.ID, (payload, context) -> {
            final int containerId = payload.id();
            final int containerSize = payload.size();
            final int entityId = payload.entityId();
            context.client().execute(() -> tallestegg.guardvillagers.client.network.GuardPacketHandler.openGuardInventory(
                    new GuardOpenInventoryPacket(containerId, containerSize, entityId)));
        });
    }
